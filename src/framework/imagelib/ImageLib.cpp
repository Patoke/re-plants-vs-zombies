...
constexpr std::array<T, N> createLUT(auto fn) {
	std::array<T, N> ret;
	...
	if (theDoImageSanding && anImage != NULL) {
		{
			...
			for (int y = 0; y < anImage->mHeight; y++)
			{
				for (int x = 0; x < anImage->mWidth; x++)
				{
					...
					{
						*aBitsPtr = AverageNearByPixels(anImage.get(), aTempBuffer, x, y);  // 计算该点周围非透明像素的平均颜色
					}
		...
		for (int y = 0; y < anImage->mHeight; y++)
		{
			for (int x = 0; x < anImage->mWidth; x++)
			{
				...
				*aBitsPtr = 
					...
					linearToSRGBLut[((alpha * r) >> 22)] << 16 |
					linearToSRGBLut[((alpha * g) >> 22)] <<  8 |
					linearToSRGBLut[((alpha * b) >> 22)]       |
					(*aBitsPtr & 0xFF000000);
